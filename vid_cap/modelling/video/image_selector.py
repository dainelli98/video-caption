...
class ImageSelector(object):
    ...
    def select_best_frames(self, input_key_frames, number_of_frames):
        ...
        min_brightness_values = np.arange(10.0, -0.01, -self.brightness_step)
        max_brightness_values = np.arange(90.0, 100.01, self.brightness_step)
        min_entropy_values = np.arange(1.0, -0.01, -self.entropy_step)
        max_entropy_values = np.arange(10.0, 10.01, self.entropy_step)
