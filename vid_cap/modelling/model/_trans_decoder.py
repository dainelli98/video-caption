...
class TransformerNet(nn.Module):
    ...
    def forward(self, src: torch.Tensor, tgt: torch.Tensor, has_mask=True) -> torch.Tensor:
        ...
        if has_mask:
            device = tgt.device
            if self.tgt_mask is None or self.tgt_mask.size(0) != len(tgt[0]): #as we're working with batch
